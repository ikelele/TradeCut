...
// Минимальный перепад яркости между соседними пикселями, чтобы считать его
// краем. Ниже — это шум сжатия, а не линия.
const EDGE_THRESHOLD = 12
// Какую долю полосы должен пройти перепад. Настоящий разделитель идёт насквозь.
...
function verticalLinesInBand(gray, width, bandTop, bandHeight) {
  const hits = new Int32Array(width - 1)
  for (let y = bandTop; y < bandTop + bandHeight; y++) {
    const row = y * width
    for (let x = 0; x < width - 1; x++) {
      if (Math.abs(gray[row + x + 1] - gray[row + x]) >= EDGE_THRESHOLD) hits[x]++
    }
  }
  const needed = bandHeight * MIN_COVERAGE
  const found = []
...
function horizontalLines(gray, width, height) {
  const found = []
  for (let y = 0; y < height - 1; y++) {
    ...
    for (let x = 0; x < width; x++) {
      if (Math.abs(gray[next + x] - gray[row + x]) >= EDGE_THRESHOLD) hits++
    }
    if (hits / width >= MIN_COVERAGE) found.push(y + 1)
  }
  return clusterPositions(found)
}
...
function detectPanelGuides(frame) {
  ...
  for (let top = 0; top + BAND_HEIGHT < height; top += BAND_STEP) {
    const lines = verticalLinesInBand(gray, width, top, BAND_HEIGHT)
  ...
  return {
    vertical: variants[0] || [],
    horizontal: horizontalLines(gray, width, height),
    variants
  }
}
